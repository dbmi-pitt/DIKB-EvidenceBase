...
s AS (
SELECT ca.id, cb.claim_text, c.concept_name, q.concept_code, c.vocabulary_id, q.qualifier_role_concept_code, q.qualifier_role_vocabulary_id
...
o AS (
SELECT ca.id, c.concept_name, q.concept_code, c.vocabulary_id, q.qualifier_role_concept_code, q.qualifier_role_vocabulary_id
FROM ohdsi.mp_claim_annotation ca 
JOIN ohdsi.oa_claim_body cb on cb.is_oa_body_of = ca.id
...
AND (s.concept_name= '@conceptname1' AND o.concept_name = '@conceptname2')
OR (o.concept_name = '@conceptname1' AND s.concept_name = '@conceptname2');
